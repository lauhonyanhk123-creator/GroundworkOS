...
**Note (GroundworkOS-specific):** GroundworkOS defaults roleless users to `"admin"`, so fresh test signups come out as admin (full access). The role-gating limitations below still apply generically — to test a *low*-privilege path, explicitly assign a `manager`/`foreman` role rather than relying on the default.
