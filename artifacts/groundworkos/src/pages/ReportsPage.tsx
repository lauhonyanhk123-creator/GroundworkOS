...
const CustomTooltip = ({ active, payload, label }: any) => {
  ...
  return (
    <div className="px-3 py-2 rounded text-xs" style={{ backgroundColor: '#eeeae4', border: '1px solid #3a3a3a', fontFamily: "'DM Mono', monospace" }}>
        ...
        <>
          <div className="flex items-start gap-3 p-4 rounded-lg" style={{ backgroundColor: '#eeeae4', border: '1px solid #d9d4ce' }}>
            <div className="flex-1">
              <p className="text-xs font-medium mb-1" style={{ color: '#a8a099' }}>Construction Industry Scheme (CIS)</p>
                    ...
                    <tbody>
                      {monthReturns.map(r => (
                        <tr key={r.id} style={{ borderBottom: '1px solid #141414' }}>
                          ...
                          <td className="py-2.5 px-3 text-sm" style={{ color: '#7a7469', fontFamily: "'DM Mono', monospace" }}>—</td>
                          <td className="py-2.5 px-3 text-sm" style={{ color: '#a8a099', fontFamily: "'DM Mono', monospace" }}>{r.deduction_rate}%</td>
                          <td className="py-2.5 px-3 text-sm" style={{ color: '#a8a099', fontFamily: "'DM Mono', monospace" }}>{formatCurrency(r.gross_payment)}</td>
                          <td className="py-2.5 px-3 text-sm" style={{ color: '#e03a3a', fontFamily: "'DM Mono', monospace" }}>-{formatCurrency(r.deduction_amount)}</td>
                          <td className="py-2.5 px-3 text-sm font-medium" style={{ color: '#181410', fontFamily: "'DM Mono', monospace" }}>{formatCurrency(r.net_payment)}</td>
                        </tr>
                      ))}
                    </tbody>
                    <tfoot>
                      <tr style={{ borderTop: '1px solid #1a1a1a' }}>
                        ...
                        <td className="py-2.5 px-3 text-sm font-medium" style={{ color: '#a8a099', fontFamily: "'DM Mono', monospace" }}>{formatCurrency(monthReturns.reduce((s, r) => s + r.gross_payment, 0))}</td>
                        ...
                        <td className="py-2.5 px-3 text-sm font-medium" style={{ color: '#181410', fontFamily: "'DM Mono', monospace" }}>{formatCurrency(monthReturns.reduce((s, r) => s + r.net_payment, 0))}</td>
                      </tr>
                    </tfoot>
                  </table>
                </div>
      ...
      {tab === 'ratebook' && (
        <>
          <div className="flex items-center gap-3">
            <select value={rateCategory} onChange={e => setRateCategory(e.target.value)} className="py-1.5 px-3 rounded-md text-sm focus:outline-none" style={{ backgroundColor: '#fafaf8', border: '1px solid #1a1a1a', color: '#a8a099' }}>
          ...
          {rateCategories.filter(c => c !== 'all').map(cat => {
            ...
            return (
              <Panel key={cat} title={cat} actions={<span className="text-xs" style={{ color: '#7a7469', fontFamily: "'DM Mono', monospace" }}>avg £{avgTotal.toFixed(2)}/unit</span>}>
                <div className="overflow-x-auto">
                  <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                    <thead>
                      <tr style={{ borderBottom: '1px solid #1a1a1a' }}>
                    ...
                    <tbody>
                      {catRates.map(r => (
                        <tr key={r.id} style={{ borderBottom: '1px solid #141414' }} className="hover:bg-[#eeeae4] transition-colors">
                          <td className="py-2.5 px-3 text-sm" style={{ color: '#181410' }}>
                            {r.description}
                            {r.notes && <div className="text-xs mt-0.5" style={{ color: '#7a7469' }}>{r.notes}</div>}
                          </td>
                          <td className="py-2.5 px-3 text-sm" style={{ color: '#7a7469', fontFamily: "'DM Mono', monospace" }}>{r.unit}</td>
                          <td className="py-2.5 px-3 text-sm" style={{ color: '#a8a099', fontFamily: "'DM Mono', monospace" }}>£{r.labour_rate.toFixed(2)}</td>
                          <td className="py-2.5 px-3 text-sm" style={{ color: '#a8a099', fontFamily: "'DM Mono', monospace" }}>£{r.material_rate.toFixed(2)}</td>
                          <td className="py-2.5 px-3 text-sm" style={{ color: '#a8a099', fontFamily: "'DM Mono', monospace" }}>£{r.plant_rate.toFixed(2)}</td>
                          <td className="py-2.5 px-3 text-sm font-medium" style={{ color: '#181410', fontFamily: "'DM Mono', monospace" }}>£{r.total_rate.toFixed(2)}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
